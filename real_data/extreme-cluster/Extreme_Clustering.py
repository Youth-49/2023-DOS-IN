...
def Extreme_Clustreing(data, neighbuorhood_radius=0.2, isGaussian=False):
    ...
    if isGaussian == False:
        ...
        for i in range(number):
            j = 0
            while j < number and b[i, j] < density_radius:
                j += 1
            density[i] = j+1
    else:
        ...
        for i in range(number-1):
            for j in range(i+1, number):
                density[i] = density[i] + \
                    np.exp(-(dist[i, j]/dc)*(dist[i, j]/dc))
                density[j] = density[j] + \
                    np.exp(-(dist[i, j]/dc)*(dist[i, j]/dc))
    ...
    for i in range(number):
        if state[i] == 0:
            j = 1
            while j < number and density[i] >= density[c[i, j]] and b[i, j] < neighbuorhood_radius:
                if density[i] == density[c[i, j]]:
                    state[c[i, j]] = 1
                j += 1

            if j >= number or b[i, j] >= neighbuorhood_radius:
                extremePoint_num += 1
    ...
    for i in range(extremePoint_num):
        ...
        j = 1
        while j < number and b[(int)(extremePoint[i][dim])][j] < neighbuorhood_radius:
